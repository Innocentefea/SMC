...
import { TEST_USER_EMAIL, FIXED_OTP_CODE, saveRedirectUrl } from '../test-data';
...
let createdClient: { id: string; redirect_uri: string } | null = null;


// Helpers

async function expectJsonResponse<T = any>(response: APIResponse): Promise<T> {
...
async function acquireAccessToken(request: APIRequestContext): Promise<string> {
...
test.describe('Connect to a Client Endpoint Validation', () => {
  test.beforeAll(async ({ request }) => {
    ...
    const clientBody = await expectJsonResponse<ClientCreateResponse>(clientResponse);
  ...
  test('Successfully connect to a valid client', async ({ request }) => {
    ...
    const body = await expectJsonResponse<ConnectClientResponse>(response);
